...
Then, having set the horizon T to 1 or 2 seconds, we have a trade-off between N and dt, where we would like to have low computational cost, but significant approximation. After trying several combinations I found that 8 timesteps of 0.2 seconds work pretty well. With this values the MPC algorithm took approximately 10ms, that is 10% of the latency length (100ms). Taking more timesteps would require to take into account that added latency into the model. This could be a future improvement.
